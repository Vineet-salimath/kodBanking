...
/* ─────────────────────────────────────────────
   SHARED TOKENS  (image reference palette)
   bg: deep navy #050d2e  →  #0a1854
   glass: rgba(255,255,255,0.08) + backdrop-blur
   accent-green: #00e5a0   accent-violet: #7c3aed
   accent-blue : #3b82f6   text: #e8eeff
───────────────────────────────────────────── */
...
/* ── LEFT BRAND PANEL ── */
.left{
  width:46%;height:100vh;flex-shrink:0;position:relative;overflow:hidden;
...
.brand-icon{
  ...
  font-weight:900;font-size:24px;color:#051a10;
  box-shadow:0 0 32px rgba(0,229,160,0.5)
}
.brand-name{font-size:26px;font-weight:800;color:var(--text)}
.hero-h{font-size:42px;font-weight:900;line-height:1.12;letter-spacing:-1px;margin-bottom:16px}
.hero-h em{font-style:normal;
  background:linear-gradient(135deg,#00e5a0,#7c3aed,#3b82f6);
  -webkit-background-clip:text;-webkit-text-fill-color:transparent;
  background-clip:text}
.hero-sub{font-size:15px;color:var(--sub);line-height:1.75;max-width:380px;margin-bottom:44px}
.feats{display:flex;flex-direction:column;gap:14px}
.feat{display:flex;align-items:center;gap:14px}
.feat-ic{width:42px;height:42px;border-radius:12px;flex-shrink:0;
  display:flex;align-items:center;justify-content:center;font-size:18px;
  background:rgba(255,255,255,0.07);border:1px solid rgba(255,255,255,0.14)}
.feat-text strong{display:block;font-size:13px;font-weight:700;color:var(--text)}
.feat-text span{font-size:12px;color:var(--muted)}

/* floating glass card */
.demo-card{
  position:absolute;bottom:44px;right:36px;width:240px;
  padding:20px;z-index:3;animation:float 4s ease-in-out infinite
}
.dc-top{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:14px}
.dc-label{font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:.08em}
.dc-badge{font-size:10px;font-weight:700;color:var(--green);
...
.dc-amount{font-size:28px;font-weight:900;margin-bottom:4px}
.dc-sub{font-size:11px;color:var(--sub)}
.dc-sparkline{display:flex;align-items:flex-end;gap:3px;height:36px;margin-top:12px}
.dc-bar{border-radius:3px 3px 0 0;flex:1}

/* ── RIGHT FORM PANEL ── */
.right{flex:1;height:100vh;overflow-y:auto;display:flex;align-items:center;justify-content:center;padding:40px 48px}
.form-box{width:100%;max-width:460px;animation:fadeUp .5s ease both}
.fh{margin-bottom:28px}
.fh h2{font-size:26px;font-weight:800;margin-bottom:6px}
.fh p{font-size:13px;color:var(--sub)}
.fh a{color:var(--green);text-decoration:none;font-weight:700}
.fh a:hover{text-decoration:underline}
.row2{display:grid;grid-template-columns:1fr 1fr;gap:12px}
.pws{display:flex;gap:4px;margin-top:5px}
.pwb{height:3px;flex:1;border-radius:2px;background:rgba(255,255,255,0.1);transition:background .3s}
...
.policy{font-size:11px;color:var(--muted);text-align:center;margin-top:14px;line-height:1.7}
.policy a{color:var(--violet2);text-decoration:none}
...
<div class="left">
  <div class="left-inner">
    <div class="brand">
      ...
      <span class="brand-name">Kodbank</span>
    </div>
    <h1 class="hero-h">Banking that<br/><em>feels different.</em></h1>
    <p class="hero-sub">Join thousands of users who trust Kodbank for secure, instant, and modern digital banking.</p>
    <div class="feats">
      <div class="feat"><div class="feat-ic">🔒</div><div class="feat-text"><strong>Military-grade Security</strong><span>256-bit encryption, JWT, SSL</span></div></div>
      <div class="feat"><div class="feat-ic">⚡</div><div class="feat-text"><strong>Instant Transfers</strong><span>Send money in under a second</span></div></div>
      <div class="feat"><div class="feat-ic">📊</div><div class="feat-text"><strong>Live Market Insights</strong><span>Real-time financial data & news</span></div></div>
      <div class="feat"><div class="feat-ic">💳</div><div class="feat-text"><strong>Virtual Debit Card</strong><span>Secure virtual card included</span></div></div>
    </div>
  </div>
  <!-- floating glass card decoration -->
  <div class="demo-card glass">
    <div class="dc-top">
      <div><div class="dc-label">Net Balance</div></div>
      <div class="dc-badge"><span class="dc-dot"></span>Live</div>
    </div>
    <div class="dc-amount">₹10,000</div>
    <div class="dc-sub">+3.2% this month</div>
    <div class="dc-sparkline">
      <div class="dc-bar" style="height:40%;background:rgba(0,229,160,0.35)"></div>
      <div class="dc-bar" style="height:65%;background:rgba(0,229,160,0.5)"></div>
      <div class="dc-bar" style="height:45%;background:rgba(0,229,160,0.35)"></div>
      <div class="dc-bar" style="height:80%;background:rgba(0,229,160,0.65)"></div>
      <div class="dc-bar" style="height:60%;background:rgba(0,229,160,0.5)"></div>
      <div class="dc-bar" style="height:90%;background:var(--green)"></div>
      <div class="dc-bar" style="height:72%;background:rgba(0,229,160,0.6)"></div>
    </div>
  </div>
</div>
    ...
    <form id="regForm" novalidate>
      ...
      <div class="field">
        ...
        <div class="pws" id="pws">
          <div class="pwb" id="pb1"></div><div class="pwb" id="pb2"></div>
          <div class="pwb" id="pb3"></div><div class="pwb" id="pb4"></div>
        </div>
      ...
      <button type="submit" class="btn-green" id="submitBtn" style="width:100%;margin-top:6px;padding:15px">Create Account</button>
    </form>
    <p class="policy">By continuing you agree to our <a href="#">Terms of Service</a> and <a href="#">Privacy Policy</a></p>
  </div>
</div>
...
document.getElementById('password').addEventListener('input',function(){
  ...
  const c=['','#ff5c7a','#fbbf24','#00e5a0','#7c3aed'];
  const lb=['','Weak','Fair','Good','Strong'];
  [1,2,3,4].forEach(i=>{document.getElementById('pb'+i).style.background=i<=s?c[s]:'rgba(255,255,255,.1)';});
...
function cl(){['username','email','phone','password','cpw'].forEach(id=>se(id,''));alertEl.className='alert';}
function validate(d){let ok=true;
  if(!d.u||!/^[a-zA-Z0-9_]{3,80}$/.test(d.u)){se('username','3-80 chars: letters, numbers, underscores');ok=false;}
  if(!d.e||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(d.e)){se('email','Enter a valid email');ok=false;}
  if(!d.ph||!/^[0-9+\-\s]{7,25}$/.test(d.ph)){se('phone','Enter a valid phone');ok=false;}
  if(!d.pw||d.pw.length<8||!/[A-Z]/.test(d.pw)||!/[a-z]/.test(d.pw)||!/\d/.test(d.pw)){se('password','Min 8 chars with upper, lower & digit');ok=false;}
  if(d.pw!==d.cpw){se('cpw','Passwords do not match');ok=false;}
  return ok;
}
function setLoad(on){submitBtn.disabled=on;submitBtn.innerHTML=on?'<span class="spinner"></span>Creating...':'Create Account';}
function showAlert(m,t){alertEl.innerHTML='<span>'+(t==='success'?'checkmark':'x')+'</span> '+m;alertEl.className='alert '+t+' show';}
form.addEventListener('submit',async e=>{
  e.preventDefault();cl();
  const d={u:document.getElementById('username').value.trim(),
  ...
  if(!validate(d))return;setLoad(true);
  try{
    const res=await fetch(API+'/register',{method:'POST',headers:{'Content-Type':'application/json'},
      credentials:'include',body:JSON.stringify({username:d.u,email:d.e,phone:d.ph,password:d.pw,role:'customer'})});
    const b=await res.json();
    if(!res.ok)showAlert(Array.isArray(b.errors)?b.errors.join(' - '):b.message,'error');
    else{showAlert(b.message+' Redirecting...','success');setTimeout(()=>location.href='login.html',1800);}
...
html,body{height:100%;overflow:hidden}
body{display:flex}
.left{width:46%;height:100vh;flex-shrink:0;position:relative;overflow:hidden;
...
.brand{display:flex;align-items:center;justify-content:center;gap:14px;margin-bottom:44px}
.brand-icon{width:54px;height:54px;border-radius:16px;background:var(--green-g);
  display:flex;align-items:center;justify-content:center;font-weight:900;font-size:24px;color:#051a10;
  box-shadow:0 0 32px rgba(0,229,160,0.5)}
.brand-name{font-size:27px;font-weight:800}
.hero-h{font-size:38px;font-weight:900;line-height:1.15;letter-spacing:-.8px;margin-bottom:14px}
.hero-h em{font-style:normal;background:linear-gradient(135deg,#00e5a0,#7c3aed);
  -webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
.hero-sub{font-size:14px;color:var(--sub);line-height:1.75;max-width:310px;margin:0 auto 40px}

.balance-card{
  width:100%;max-width:300px;margin:0 auto;
  padding:22px 24px;border-radius:20px;
  background:linear-gradient(135deg,rgba(124,58,237,0.25),rgba(0,229,160,0.12));
  border:1px solid rgba(255,255,255,0.2);backdrop-filter:blur(24px);
  box-shadow:0 20px 60px rgba(0,0,0,0.35),0 0 0 1px rgba(255,255,255,0.08) inset;
  animation:float 4s ease-in-out infinite
}
.bc-top{display:flex;justify-content:space-between;align-items:center;margin-bottom:16px}
.bc-lbl{font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:.1em}
.bc-ico{font-size:20px}
.bc-amount{font-size:32px;font-weight:900;letter-spacing:-.5px;
  background:linear-gradient(135deg,#fff,rgba(255,255,255,.7));
  -webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
.bc-sub{font-size:12px;color:var(--green);margin-top:4px;font-weight:600}
.bc-wave{margin-top:16px;overflow:hidden;height:40px;position:relative}
.bc-wave svg{position:absolute;bottom:0;width:100%}

.right{flex:1;height:100vh;overflow-y:auto;display:flex;align-items:center;justify-content:center;padding:40px 56px}
.form-box{width:100%;max-width:400px;animation:fadeUp .5s ease both}
.fh{margin-bottom:28px}
.fh h2{font-size:26px;font-weight:800;margin-bottom:6px}
.fh p{font-size:13px;color:var(--sub)}
.fh a{color:var(--green);text-decoration:none;font-weight:700}
.fh a:hover{text-decoration:underline}
.pw-row{display:flex;justify-content:flex-end;margin-top:-12px;margin-bottom:18px}
.pw-row a{font-size:12px;color:var(--muted);text-decoration:none;transition:color .2s}
.pw-row a:hover{color:var(--green)}
@media(max-width:860px){.left{display:none}.right{padding:28px 20px}html,body{height:auto;overflow:auto}}
</style>
</head>
<body>
...
<div class="left">
  <div class="left-inner">
    <div class="brand"><div class="brand-icon">K</div><span class="brand-name">Kodbank</span></div>
    <h1 class="hero-h">Welcome<br/><em>back.</em></h1>
    <p class="hero-sub">Your money is safe, growing, and always accessible.</p>
    <div class="balance-card">
      <div class="bc-top">
        <div><div class="bc-lbl">Account Balance</div></div>
        <div class="bc-ico">💳</div>
      </div>
      <div class="bc-amount">₹10,000.00</div>
      <div class="bc-sub">↑ +3.2% this month</div>
      <div class="bc-wave">
        <svg viewBox="0 0 300 40" preserveAspectRatio="none" fill="none">
          <path d="M0 25 C50 5 100 40 150 20 C200 0 250 35 300 15 L300 40 L0 40 Z" fill="rgba(0,229,160,0.15)"/>
          <path d="M0 30 C60 12 110 38 170 22 C220 8 270 32 300 18 L300 40 L0 40 Z" fill="rgba(124,58,237,0.12)"/>
          <path stroke="#00e5a0" stroke-width="2" d="M0 25 C50 5 100 40 150 20 C200 0 250 35 300 15" fill="none"/>
          <path stroke="#7c3aed" stroke-width="1.5" d="M0 30 C60 12 110 38 170 22 C220 8 270 32 300 18" fill="none" stroke-dasharray="4 3"/>
        </svg>
      </div>
    </div>
  </div>
...
<div class="right">
  <div class="form-box">
    <div class="fh">
      <h2>Sign in to Kodbank</h2>
      <p>New here? <a href="register.html">Create a free account</a></p>
    </div>
    <div id="alert" class="alert"></div>
    <form id="loginForm" novalidate>
      ...
      <button type="submit" class="btn-green" id="submitBtn" style="width:100%;padding:15px">Sign In</button>
    </form>
  </div>
</div>
<script>
...
const alertEl=document.getElementById('alert');
const submitBtn=document.getElementById('submitBtn');
function showAlert(m,t){alertEl.innerHTML='<span>'+(t==='success'?'ok':'x')+'</span> '+m;alertEl.className='alert '+t+' show';}
function se(id,m){const e=document.getElementById('err-'+id);if(e){e.textContent=m;e.classList[m?'add':'remove']('show');}}
function cl(){['username','password'].forEach(id=>se(id,''));alertEl.className='alert';}
function setLoad(on){submitBtn.disabled=on;submitBtn.innerHTML=on?'<span class="spinner"></span>Signing in...':'Sign In';}
form.addEventListener('submit',async e=>{
  e.preventDefault();cl();
  const u=document.getElementById('username').value.trim();
  const p=document.getElementById('password').value;
  let ok=true;
  if(!u){se('username','Username is required');ok=false;}
  if(!p){se('password','Password is required');ok=false;}
  if(!ok)return;setLoad(true);
  try{
...
<nav class="sidebar">
  ...
  <div class="sb-nav">
    ...
    <a class="nav-item active" data-page="dashboard" onclick="nav(this,'dashboard')"><span class="nav-icon">🏠</span><span>Dashboard</span></a>
    <a class="nav-item" data-page="transfer" onclick="nav(this,'transfer')"><span class="nav-icon">↗️</span><span>Transfer</span></a>
    <a class="nav-item" data-page="transactions" onclick="nav(this,'transactions')"><span class="nav-icon">📋</span><span>Transactions</span></a>
    <div class="sb-section">Account</div>
    <a class="nav-item" data-page="cards" onclick="nav(this,'cards')"><span class="nav-icon">💳</span><span>My Card</span></a>
    <a class="nav-item" data-page="profile" onclick="nav(this,'profile')"><span class="nav-icon">👤</span><span>Profile</span></a>
    <div class="sb-section">Insights</div>
    <a class="nav-item" data-page="market" onclick="nav(this,'market')"><span class="nav-icon">📈</span><span>Market</span><span class="nav-pill">Live</span></a>
  </div>
  <div class="sb-bottom">
    <div class="sb-user">
...
<div class="main">
  <div class="topbar">
    ...
    <div class="topbar-right">
      <div style="position:relative">
        <div class="icon-btn" id="notifBtn" title="Notifications">🔔<div class="badge" id="notifBadge" style="display:none"></div></div>
      ...
      <div style="position:relative">
        ...
        <div class="glass2 settings-dropdown" id="settingsDropdown">
          <div class="setting-item" onclick="nav(document.querySelector('[data-page=profile]'),'profile');closeDropdowns()"><span>👤</span> My Profile</div>
          <div class="setting-item" onclick="toggleTheme()"><span>🎨</span> Toggle Theme</div>
          <div class="setting-item" onclick="exportData()"><span>📤</span> Export Data</div>
  ...
  <div class="content">
    ...
    <div class="page active" id="page-dashboard">
      <div class="greet-row">
        <div class="greet-left"><h1>Hello, <em id="greetName">...</em> 👋</h1><p>Here's your financial overview</p></div>
      ...
      <div class="balance-hero">
        ...
        <div class="bh-actions">
          <button class="bh-btn primary" onclick="nav(document.querySelector('[data-page=transfer]'),'transfer')">↗️ Send Money</button>
          <button class="bh-btn secondary" onclick="nav(document.querySelector('[data-page=transactions]'),'transactions')">📋 History</button>
          <button class="bh-btn ghost" onclick="refreshBal()">↻ Refresh</button>
          <button class="bh-btn ghost" id="hideBal" onclick="toggleBal()">👁 Hide</button>
        </div>
      </div>
      <div class="grid4">
        <div class="stat-card"><div class="stat-ic" style="background:rgba(0,229,160,0.1)">💰</div><div><div class="stat-l">Total Credit</div><div class="stat-v up" id="stCredit">...</div><div class="stat-ch up" id="stCreditC"></div></div></div>
        <div class="stat-card"><div class="stat-ic" style="background:rgba(255,92,122,0.1)">💸</div><div><div class="stat-l">Total Debit</div><div class="stat-v down" id="stDebit">...</div><div class="stat-ch down" id="stDebitC"></div></div></div>
        <div class="stat-card"><div class="stat-ic" style="background:rgba(251,191,36,0.1)">⏳</div><div><div class="stat-l">Pending</div><div class="stat-v" style="color:var(--yellow)" id="stPending">...</div><div class="stat-ch neu" id="stPendingC"></div></div></div>
        <div class="stat-card"><div class="stat-ic" style="background:rgba(59,130,246,0.1)">📊</div><div><div class="stat-l">Transactions</div><div class="stat-v" style="color:var(--blue2)" id="stCount">...</div><div class="stat-ch neu">Total</div></div></div>
      </div>
      <div class="grid23">
        <div class="card">
          <div class="card-title">📋 Recent Transactions<a onclick="nav(document.querySelector('[data-page=transactions]'),'transactions')">View all</a></div>
    ...
    <div class="page" id="page-transfer">
      <div class="page-header"><div class="page-h1">↗️ Send Money</div></div>
    ...
    <div class="page" id="page-transactions">
      <div class="page-header"><div class="page-h1">📋 Transaction History</div><div style="font-size:12px;color:var(--muted)" id="txCountLbl"></div></div>
    ...
    <div class="page" id="page-market">
      <div class="page-header"><div class="page-h1">📈 Market and Insights</div></div>
      <div class="market-grid">
        <div>
          <div class="section-title">📰 Financial News <span class="pill pill-green" style="margin-left:6px">Live</span></div>
          <div id="newsList"><div class="loading-center"><span class="spinner"></span></div></div>
        </div>
        <div>
          <div class="section-title">🌍 World Bank Datasets</div>
          <div id="wbList"><div class="loading-center"><span class="spinner"></span></div></div>
          <div class="divider"></div>
          <div class="section-title">💹 Market Tickers</div>
...
const NOTIFS=[
  {icon:'💰',text:'Your account balance was updated.',time:'Just now',color:'rgba(0,229,160,0.1)',read:false},
  {icon:'🔒',text:'New login detected from your device.',time:'2 min ago',color:'rgba(59,130,246,0.1)',read:false},
  {icon:'📊',text:'Monthly statement is ready.',time:'1 hr ago',color:'rgba(124,58,237,0.1)',read:false},
];
...
async function loadTx(){
  try{
    const r=await fetch(API+'/transactions?limit=100&offset=0',{credentials:'include'});
    const b=await r.json();
...
async function doTransfer(){
  ...
  try{
    const r=await fetch(API+'/transfer',{method:'POST',headers:{'Content-Type':'application/json'},
      credentials:'include',body:JSON.stringify({recipient:rec,amount:amt,description:desc})});
    const b=await r.json();
    if(!r.ok){showTfAlert(b.message||'Transfer failed.','error');}
      ...
      addNotif('↗️','Transfer of '+inr(amt)+' to '+rec+' completed.','Just now','rgba(0,229,160,0.1)');
      loadTx();
    }
...
fs.writeFileSync(__dirname+'/register.html', register, 'utf-8');
fs.writeFileSync(__dirname+'/login.html', login, 'utf-8');
fs.writeFileSync(__dirname+'/dashboard.html', dashboard, 'utf-8');
console.log('All pages generated');
console.log('  register.html :', fs.statSync(__dirname+'/register.html').size, 'bytes');
console.log('  login.html    :', fs.statSync(__dirname+'/login.html').size, 'bytes');
console.log('  dashboard.html:', fs.statSync(__dirname+'/dashboard.html').size, 'bytes');